...
class HowaHowa {
  ...
  dataInit(name, argdata) {
    const initdata = {
      taste: name, color: "red", pos: { x: 0, y: 0 }, strength: 0, sharpness: 0
    };
    if (argdata.hasOwnProperty('color')) {
      initdata.color = argdata.color;
    }
    if (argdata.hasOwnProperty('pos')) {
      if (argdata.pos.hasOwnProperty('x') && argdata.pos.hasOwnProperty('y')) {
        initdata.pos = argdata.pos;
      }
    }
    if (argdata.hasOwnProperty('strength')) {
      initdata.strength = argdata.strength;
    }
    if (argdata.hasOwnProperty('sharpness')) {
  ...
  makeHowa(draw) {
    ...
    const path = nested.path(this.plotData(0)).fill({ color: this.data.color, opacity: 0.8 }).move(0, 0).size(100);
  ...
  dragMove(event) {
    ...
    this.howa.dragTime++;//移動回数を追加していく
  ...
  dragEnd(event, draw) {
    ...
    if (this.howa.dragTime < 8 && this.holdTime <= 1000) {//移動してない(dragMove=3)+ロングタップでないときはクリックイベント
      console.log("click");
      this.data.sharpness = (this.data.sharpness + 1) % this.plotData();
      this.howa.pathElem.animate(100).plot(this.plotData(this.data.sharpness)).move(0, 0).size(100);
      if (!this.clCount) {
        this.clCount++;
        setTimeout(() => {
          this.clCount = 0;
